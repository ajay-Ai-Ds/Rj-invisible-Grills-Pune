...
export default function MapFrame({ areaName = "Pune", className = "" }: MapFrameProps) {
  ...
  const mapSearchUrl = `https://maps.google.com/maps?q=${encodeURIComponent(
    areaName.toLowerCase() === "pune" ? "Pune, Maharashtra, India" : `${areaName}, Pune, Maharashtra, India`
  )}&t=&z=14&ie=UTF8&iwloc=&output=embed`;

  return (
    <div className={`relative w-full h-[350px] rounded-3xl overflow-hidden shadow-xs border border-slate-200 ${className}`}>
      <iframe
        title={`RJ Invisible Grills Service Coverage Map - ${areaName}`}
        src={mapSearchUrl}
        width="100%"
        height="100%"
        style={{ border: 0 }}
